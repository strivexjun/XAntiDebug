...
BOOL CRC32File(LPCTSTR lpszFileName, unsigned char digest[16])
{
    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        HANDLE hFileMap = CreateFileMapping(hFile, NULL, PAGE_READONLY, dwFileSizeHigh, dwFileSizeLow, NULL);
        if (hFileMap)
        {
            LPBYTE lpbMapAddress = (LPBYTE)MapViewOfFile(hFileMap, FILE_MAP_READ, 0, 0, 0);

            if (lpbMapAddress)
            {
                bRet=crc32(lpbMapAddress, dwFileSizeLow);
